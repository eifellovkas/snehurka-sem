...
public class Kabelka 
{
    ...
    public Map<String, Vec> seznamVeci2; //Mapa je vhodnejsi pro GUI, kvuli zachovani puvodni adventury zanechavam funkcionalitu setu
    ...
    public boolean vlozVec (Vec v)
    {
        if(jeMistoVKabelce() && (v.jePrenositelna())) {
            seznamVeci.add(v);
            seznamVeci2.put(v.getNazev(),v); //opet kvuli mapě z předchozího komentáře
            return true;
        }
        return false;
    }
    ...
    public String veciVKabelce2(){
        String result = "";
        if(seznamVeci2.isEmpty()){
            result += "Kabelka je prázdná";
        }
        else{
            for (Vec vec : seznamVeci){
